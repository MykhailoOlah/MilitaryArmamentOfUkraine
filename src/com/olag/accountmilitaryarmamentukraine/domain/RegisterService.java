...
public class RegisterService {
    ...
    public static void registration() {
        ...
        System.out.print("Введiть логiн\uD83D\uDE4B\uD83C\uDFFC\u200D♂\uFE0F: ");
        ...
        try {
            for (User usersList : users) {
                if (Objects.equals(usersList.getName(), userLogin) || Objects.equals(usersList.getEmail(), userEmail)) {
                    System.out.println("Такий аккаунт iснує або введена електронна пошта вже використовується❌");
                    registration();
                }
            }
            ...
            if (ValidationService.validate(userLogin, userPassword)) {
                String verificationCode = EmailVerificationService.generateAndSendVerificationCode(userEmail);

                System.out.print("Введіть код підтвердження, відправлений на вашу електронну адресу\uD83D\uDCEC: ");
                String inputCode = scanner.next();

                Boolean verifyCode = EmailVerificationService.verifyCode(inputCode, verificationCode);
                if(verifyCode){
                    String hashedUserPassword = BCrypt.withDefaults()
                        .hashToString(12, userPassword.toCharArray());
                    User newUser = new User(userLogin, hashedUserPassword, userEmail, false, "User");
                    users.add(newUser);
                    ...
                    System.out.println("Помилка! Верифікаційний код введено неправильно, або невчасно. Повторіть спробу❌");
                }
                Menu.startMenu();
            } else {
